...
router.post("/login", async (req, res) => {
  try {
    ...
    // ❌ Prevent user login via admin login
    const user = await User.findOne({ email });
    if (user) {
      return res.status(401).json({
        success: false,
        message: "Access denied. Users cannot log in from admin panel.",
      });
    }
...
// Toggle pooja active status
router.patch("/poojas/:id/status", adminController.updatePoojaStatus);
